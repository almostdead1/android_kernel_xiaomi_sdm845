...
TRACE_EVENT(sched_task_util,
	...
	TP_PROTO(struct task_struct *p, int next_cpu, int backup_cpu,
		 ...
		 bool placement_boost, int rtg_cpu, u64 start_t, bool is_uxtop),

	TP_ARGS(p, next_cpu, backup_cpu, target_cpu, sync, need_idle, fastpath,
		placement_boost, rtg_cpu, start_t, is_uxtop),
	...
	TP_printk("pid=%d comm=%s util=%lu prev_cpu=%d next_cpu=%d backup_cpu=%d target_cpu=%d sync=%d need_idle=%d fastpath=%d placement_boost=%d rtg_cpu=%d latency=%llu is_uxtop=%d",
		__entry->pid, __entry->comm, __entry->util, __entry->prev_cpu, __entry->next_cpu, __entry->backup_cpu, __entry->target_cpu, __entry->sync, __entry->need_idle,  __entry->fastpath, __entry->placement_boost, __entry->rtg_cpu, __entry->latency, __entry->is_uxtop)
);
